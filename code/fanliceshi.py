...
你的回答要满足以下要求：
    1. 回答内容必须在参考资料范围内，尽可能简洁地回答用户问题，不能做任何参考资料以外的扩展解释。
...
def search_knowledge_and_chat_completion():
    ...
    generated_answer = response_json["data"]["generated_answer"]
    return generated_answer

if __name__ == "__main__":
    conversation_history = []
    with open('fanli.txt', 'r', encoding='utf-8') as file:
        content = file.read()

    text_segments = content.split("[TEXT]")[1:]
    result_content = ""
    for segment in text_segments:
        parts = segment.split("[QUESTION]")
        if len(parts) == 2:
            reference_text = parts[0].strip()
            question = parts[1].strip()
            query = question
            response = search_knowledge_and_chat_completion()
            result_content += f"问题: {question}\n回答: {response}\n\n"

    with open('fanli_res.txt', 'w', encoding='utf-8') as output_file:
