...
# 定义评估提示
evaluation_prompt = """请评估以下预测答案和参考答案的匹配程度，按照以下标准进行判断：
- 若预测答案和参考答案完全相同，判断结果为“完全一致”。
- 若参考答案包含在预测答案中，且预测答案的额外信息都来自于给定的文本内容，判断结果为“匹配”。
- 若不满足上述两种情况，判断结果为“不匹配”。

请在<判断结果>标签内写下判断结果，判断结果只能是“完全一致”“匹配”“不匹配”这三个固定回答。然后在<理由>标签内详细说明做出该判断的理由。
    ...
    with open(file_path, 'r', encoding='utf-8') as file:
        ...
        while i < len(lines):
            if lines[i].startswith('[TEXT]'):
                text = lines[i].strip().replace('[TEXT]', '').strip()
                i += 1
            ...
            i += 1
    return questions, answers, texts
...
# 让大模型进行评估
def evaluate_with_model(text, predicted_answer, reference_answer):
    eval_query = evaluation_prompt.format(text, predicted_answer, reference_answer)
    messages = [
        {
            "role": "system",
            "content": ""
    ...
    try:
        ...
        if "generated_answer" in response_json["data"]:
            eval_result = response_json["data"]["generated_answer"]
        else:
            eval_result = response_json["data"]["choices"][0]["message"]["content"]
        return eval_result
    except (json.JSONDecodeError, KeyError) as e:
        print(f"评估时出错，响应内容: {response_text}")
        print(f"错误信息: {e}")
        return "评估出错"
...
if __name__ == "__main__":
    ...
    questions, answers, texts = read_test_answer(test_file_path)
    rouge_l_scores = []
    with open('evaluation_res.txt', 'w', encoding='utf-8') as file:
        for question, answer, text in zip(questions, answers, texts):
            query = question
            ...
            model_eval_result = evaluate_with_model(text, predicted_answer, answer)
            file.write(f"问题: {question}\n")
            file.write(f"预测答案: {predicted_answer}\n")
            file.write(f"标准答案: {answer}\n")
            file.write(f"ROUGE-L: {rouge_l_score}\n")
            file.write(f"大模型评估结果: {model_eval_result}\n")
            file.write("-" * 50 + "\n")
        if rouge_l_scores:
            average_rouge_l = sum(rouge_l_scores) / len(rouge_l_scores)
            file.write(f"所有ROUGE - L的平均值: {average_rouge_l}\n")